...
const FEATURES: FeatureSlide[] = [
...

  {
    ...
    subtitle: "Learn how Rustyn works under the hood",
    bullets: [
      "Architecture & technical breakdown",
      "Privacy, audio intelligence & more",
    ],
    type: "support",
    actionLabel: "Start the app",
    url: "doc.md",
  },
];
...
export const FeatureSpotlight: React.FC = () => {
  ...
  const [interestState, setInterestState] = useState<Record<string, boolean>>(
    () => {
      try {
        const saved = localStorage.getItem("rustyn_feature_interest");
  ...
  const handleActionClick = (e: React.MouseEvent) => {
    ...
    if (isSupport && currentFeature.url) {
      // Open the doc.md file — use Electron shell to open the local file
      if (window.electronAPI && window.electronAPI.openExternal) {
        // Construct file:// path to doc.md relative to the app root
        const docPath = new URL(currentFeature.url, window.location.href).href;
        window.electronAPI.openExternal(docPath);
      } else {
    ...
    localStorage.setItem("rustyn_feature_interest", JSON.stringify(newState));
  };
  ...
  const buttonBg = isSupport
    ? isInterested
      ...
      : "rgba(255, 240, 240, 0.08)" // Warmer tint
    : isInterested
            ...
            <div className="relative h-full w-full flex flex-col items-center justify-center">
              ...
              >
                ...
                {currentFeature.bullets && (
                  <div
                    className={`flex flex-col w-full max-w-[340px] gap-1 items-center translate-y-2.5`}
                  >
                    {currentFeature.bullets.map((bullet, idx) => (
                      <div
                        key={idx}
                        className={`flex items-center justify-center group/item transition-transform duration-200 px-2`}
                      >
                        <span
                          className={`${isSupport ? "text-[12px] leading-relaxed font-medium opacity-100" : "text-[12.5px] leading-snug font-medium"} text-[#E6C46A]`}
                          style={{
                            letterSpacing: isSupport ? "0.01em" : "-0.01em",
                          }}
                        >
                          {bullet}
                        </span>
                  ...
                  <motion.button
                    ...
                    style={
                      isSupport
                        ? {
                            background:
                              "linear-gradient(180deg, #F1D88B 0%, #E6C87A 100%)",
                            boxShadow: `
